...
def aft_wrapper(prior: Potential, target: Potential, flow: str):
    flow_object = create_flow_object(flow)
    return aft(prior, target, flow_object, full_output=True)


def craft_wrapper(prior: Potential, target: Potential, flow: str):
    flow_object = create_flow_object(flow)
    return craft(prior, target, flow_object, full_output=True)


def dlmc_wrapper(prior: Potential, target: Potential, flow: str, n_particles: int = 100):
