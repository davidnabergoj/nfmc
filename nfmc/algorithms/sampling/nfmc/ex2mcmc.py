...
class Ex2MCMC(JumpNFMC):
    def __init__(self,
        ...
        if kernel is None:
            kernel = Ex2MCMCKernel(event_shape, target, create_flow_object('realnvp', event_shape))
        if params is None:
    ...
    def nf_step(self, x, out):
        ...
        candidates = torch.zeros(size=(n_chains, self.params.pool_size, *event_shape)).to(x)
        log_prob_flow = torch.zeros(size=(n_chains, self.params.pool_size)).to(x)

        # sample candidates, compute log density under the target and the NF
        candidates[:, 0], log_prob_flow[:, 0] = x, flow.log_prob(x)
        candidates[:, 1:], log_prob_flow[:, 1:] = flow.sample((n_chains, pool_size - 1), return_log_prob=True)
        ...
        pool_indices = torch.tensor([
            int(torch.distributions.Categorical(logits=log_sn_weights[i]).sample(()))
