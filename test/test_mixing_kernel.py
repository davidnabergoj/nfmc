...
@pytest.mark.parametrize('n_cycles', [1, 4])
@pytest.mark.parametrize('n_steps', [5, 15])
@pytest.mark.parametrize('n_chains', [1, 4])
def test_warmup(event_shape, kernel_class, n_cycles, n_chains, n_steps):
    torch.manual_seed(0)
    ...
    z0 = torch.randn(size=(n_chains, *event_shape)) / 10
    draws = sampler.warmup(
        n_cycles=n_cycles,
    ...
    assert draws.as_tensor().shape == (n_cycles * n_steps, n_chains, *event_shape)
    assert torch.all(torch.isfinite(draws.as_tensor()))
...
def test_sample(event_shape, kernel_class, n_chains, n_steps):
    ...
    sampler = PreconditionedMCMCSampler(mixing_kernel)
    z0 = torch.randn(size=(n_chains, *event_shape)) / 10
    ...
    assert draws.as_tensor().shape == (n_steps, n_chains, *event_shape)
    assert torch.all(torch.isfinite(draws.as_tensor()))
